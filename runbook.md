...
Evidencia clave:
nc -vz 127.0.0.1 80 → succeeded y ss -tlnp confirmaron Nginx sano, escuchando y aceptando conexiones — descartando problema de red hacia el proxy.
error.log de Nginx mostró el mensaje exacto: connect() failed (111: Connection refused) while connecting to upstream ... upstream: "http://127.0.0.1:8000/health" — código 111 = ECONNREFUSED a nivel de sistema operativo.
ss -tlnp | grep ":8000" no devolvió resultados — ningún proceso escuchando en el puerto del backend.
systemctl status devops-journey confirmó inactive (dead), proceso detenido con signal=TERM.
...
🔵 Seguridad
1
40 min (compartido con Red/Config en #006)
🟣 Recursos
0
—
